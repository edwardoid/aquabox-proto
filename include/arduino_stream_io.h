...
namespace aquabox
{
    namespace proto
    {
        ...
        public:
            ...
            void init(Stream* stream) { m_stream = stream; }

            VIRTUAL_FN bool good() const { return true; }

            void setPollTimeout(int timeout) { m_pollTimeout = timeout; }

            VIRTUAL_FN buffer_length_t available() const { return (buffer_length_t) m_stream->available(); }
