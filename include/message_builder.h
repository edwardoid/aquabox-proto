...
namespace aquabox
{
    namespace proto
    {
        ...
        {
            static inline void setSerial(Message& msg, const uint8_t* serial) { memcpy(&msg.serial, serial, SERIAL_LEN); }

            static inline void setToken(Message& msg, uint8_t* token) { memcpy(&msg.token, token, TOKEN_LEN); }
